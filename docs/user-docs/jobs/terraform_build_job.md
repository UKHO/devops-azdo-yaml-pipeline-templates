...
If you see this error, the Terraform CLI installation failed. Check:

- Network connectivity to download Terraform binaries
- Correct `TerraformVersion` format (e.g., '1.9.0', not 'latest')
- Agent has write permissions to temporary directories

### Validation Failures

If `terraform validate` fails:

- Check Terraform syntax in your configuration files
- Ensure all required variables are defined or have defaults
- Look for file encoding issues (Terraform requires UTF-8)

### Files Not Included in Artifact

If expected files aren't in the artifact:

- Verify `RelativePathToTerraformFiles` path is correct and relative to repository root
- Check that `AdditionalFilesToPackage` glob patterns match the files you expect
- Ensure file names match the patterns (glob patterns are case-sensitive on Linux agents)

## Related Templates

- [**terraform_deploy_job.md**](terraform_deploy_job.md) — Deploy the built artifact to infrastructure environments
- [**terraform_gated_deployment_job.md**](terraform_gated_deployment_job.md) — Plan, verify, and apply Terraform changes
  with manual gates
- [**Terraform Pipeline**](../pipelines/terraform_pipeline.md) — Complete end-to-end pipeline using this job
